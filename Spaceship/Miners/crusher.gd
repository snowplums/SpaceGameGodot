...
var crushed_copper = 0 # Amount of crushed copper
...
func start_crushing():
	if crushed_copper != max_crushed_ores:
		$CrusherTimer.start()
...
func _process(_delta):
	if in_radius and Input.is_action_just_pressed("E") and network.get_node(str(multiplayer.get_unique_id())).geode_amt >= 1: #Put geode IN
		print("PUT GEODE IN")
		rpc("crush_geode")
	elif in_radius and Input.is_action_just_pressed("E") and crushed_copper >= 1: #Take crushed geode OUT
...
func _on_crusher_timer_timeout():
	...
	if geode >= 1:
		geode -= 1
		crushed_copper += 1
		if (crushed_copper) != max_crushed_ores: #In crushed_copper add all other ores as well
			start_crushing()
		print("Crushed: " + str(crushed_copper))
		print("Geode: " + str(geode))
